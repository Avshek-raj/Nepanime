...
export const AnimeSearch = () => {
    ...
    const { data, isPending, error } = useQuery({
        queryKey: ['anime-search', keyword, currentPage],
        queryFn: () => fetchAnime(`/${keyword}`),
        enabled: !!keyword
    });
    ...
    if (error) return <p className="text-center text-white/70 mt-10">Error fetching search results.</p>;
    
    if (!keyword) {
        return <div className="text-center text-white/70 mt-10 px-5">Please provide a search keyword.</div>;
    }
    ...
    return (
        <div className="px-5 py-10 min-h-screen">
            <h1 className="text-2xl sm:text-3xl font-bold mb-6">Search Results for "{keyword}"</h1>
            
            {isPending ? (
                <SkeletonLoader count={15} type="grid" />
            ) : results.length === 0 ? (
                <p className="text-center text-white/70">No anime found.</p>
            ) : (
                <>
